...
# 3. Water Quality Dataset Generation
with open('EarthPulseAI/ml/water_quality_dataset.csv', 'w', newline='') as f:
    ...
    for _ in range(500):
        ...
        hardness = round(random.uniform(50, 400), 1)
        solids = round(random.uniform(100, 3000), 1)
        chloramines = round(random.uniform(0, 10), 2)
        sulfate = round(random.uniform(10, 500), 1)
        conductivity = round(random.uniform(100, 1000), 1)
        oc = round(random.uniform(0, 30), 2)
        thm = round(random.uniform(0, 150), 2)
        turbidity = round(random.uniform(0, 10), 2)
        
        # Potability logic (Strictly adhering to standards for high accuracy)
        potable = 1
        if not (6.5 <= ph <= 8.5): potable = 0
        if hardness > 300: potable = 0
        if solids > 1000: potable = 0
        if chloramines > 4: potable = 0
        if sulfate > 250: potable = 0
        if turbidity > 5: potable = 0
